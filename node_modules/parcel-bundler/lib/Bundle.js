...
class Bundle {
  ...
  getHashedBundleName(contentHash) {
    ...
    let isEntry = isMainEntry || Array.from(entryAsset.parentDeps).some(dep => dep.entry);
    ...
    if (isEntry) {
      return Path.join(Path.relative(Path.dirname(entryAsset.options.mainFile), Path.dirname(entryAsset.name)), name + ext);
    }
